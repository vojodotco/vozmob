      ...
      if (header) {
        ...
        switch (element.type) {
          ...
          case 'select':
           ...
           content.append($('<select />').attr({'id':'edit-' + name + '_' + fid,'name':name +'_' + fid,'class':'swfupload-rotate', 'style':'display:none'}).append($("<option></option>").attr("value",0).text(Drupal.t('None')),$("<option></option>").attr("value",90).text(Drupal.t('90 degrees clockwise')),$("<option></option>").attr("value",180).text(Drupal.t('180 degrees')),$("<option></option>").attr("value",270).text(Drupal.t('90 degrees anti-clockwise'))));
           content.append($('<a  href="#">' + Drupal.t('Left') + '&nbsp;</a>&nbsp;').click(function(e) {
             swfuploadRotateImg(this, -90);
             swfuploadSetForm(this, -90);
             ref.updateStack(file);
             return false;
           }));
           content.append($(' <a  href="#">' + Drupal.t('Right') + '</a>&nbsp;').click(function(e) {
             swfuploadRotateImg(this, 90);
             swfuploadSetForm(this, 90);
             ref.updateStack(file);
             return false;
  ...
  ref.addFileItem = function(file) {
    ...
    for (var name in ref.instance.elements) {
      ...
      if (dom_obj.size() > 0) {
        ...
        if (dom_obj[0].tagName.toLowerCase() == 'input' || dom_obj[0].tagName.toLowerCase() == 'textarea' || dom_obj[0].tagName.toLowerCase() == 'select') {
    ...
    if (file.thumb) {
      ...
      new_file_obj.find('.sfwupload-list-mime').append($('<img />').attr({'class':'image_to_rotate','id':'edit-img-thumb_' + file.fid ,'src':file.thumb}));
      ...
      setTimeout(function() {
        $('#' + ref.queue[file.id].fid).find('.sfwupload-list-progressbar').animate({'opacity':0}, "slow", function() {
          ...
          if (ref.queue[file.id].thumb) {
            ...
            $('.sfwupload-list-mime', $('#' + ref.queue[file.id].fid)).css({'background-image': 'url()'});
            $('.sfwupload-list-mime', $('#' + ref.queue[file.id].fid)).append($('<img />').attr({'class':'image_to_rotate','id':'edit-img-thumb_' + ref.queue[file.id].fid ,'src':ref.queue[file.id].thumb}));
          };
...
$(function() {
  ...
  function swfuploadRotateImg(button, angle) {
    var rotateOption = $(button).parent().find('select.swfupload-rotate');
  ...
  function swfuploadSetForm(button, angle) {
    var previewDiv = $(button).parent().parent().parent().find('.sfwupload-list-mime'); 
